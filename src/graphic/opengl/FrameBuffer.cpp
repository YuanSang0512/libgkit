...
gkit::graphic::opengl::buffer::FrameBuffer::FrameBuffer(int width, int height) : fb_width(width), fb_height(height) {
...
gkit::graphic::opengl::buffer::FrameBuffer::FrameBuffer(FrameBuffer&& other) noexcept :
    renderer_id(other.renderer_id), fb_height(other.fb_height), fb_width(other.fb_width), left_x(other.left_x),
...
auto gkit::graphic::opengl::buffer::FrameBuffer::attach_color_texture(const gkit::graphic::opengl::Texture& texture,
                                                                      int slot) -> void {
    bind();
...
auto gkit::graphic::opengl::buffer::FrameBuffer::attach_depth_stencil(const RenderBuffer& rbo) -> void {
    bind();
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo.get_render_id());
}
